...
for row = 1+pad:sizeRow-1-pad       %go through actual image, not padding
    for column = 1+pad:sizeColumn-1-pad
        ...
        target = int32(rightPad(row-windowSize:row+windowSize, column+disparityRange-windowSize:column+disparityRange+windowSize));
        ...
        disparityLeft(row, column) = minSSD;
...
for row = 1+pad:sizeRow-1-pad       %go through actual image, not padding
    for column = 1+pad:sizeColumn-1-pad
        ...
        for offset = column-disparityRange:column+disparityRange
            ...
            difference = target-template;
            squareDiff = difference.^2;
            currentSSD = sum(squareDiff(:));
            ...
            ssdArray(offset-column+disparityRange+1) = sum(squareDiff(:));
